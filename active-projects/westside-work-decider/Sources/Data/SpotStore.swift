...
final class SpotStore: ObservableObject {
    ...
    func markVisited(_ spot: LocationSpot) {
        var recents = recentVisits.filter { $0.id != spot.id }
        recents.insert(decorate(spot: spot), at: 0)
        recentVisits = Array(recents.prefix(10))
    }
    ...
    private func loadState() async {
        ...
        do {
            ...
            recentVisits = spots.filter { decoded.recentIDs.contains($0.id) }
        } catch {
    ...
    private func persistState() {
        let state = UserState(favorites: Array(favorites), recentIDs: recentVisits.map { $0.id })
        do {
    ...
    private func applyUserState() {
        ...
        recentVisits = recentVisits.filter { existing.contains($0.id) }
    }
}
